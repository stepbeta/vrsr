...
func SaveToCache(tool string, allReleases []*github.RepositoryRelease) {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "Failed to marshal release data to json:", err)
		return
	}
	cachePath, err := GetCachePath(tool)
	if err != nil {
		fmt.Fprintln(os.Stderr, "Failed to retrieve cache path:", err)
		return
	}
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "Failed to save release data to cache:", err)
		return
	}
}
...
func ReadFromCache(tool string, limit int) (ReleasesData, error) {
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "Failed to read cache data from file:", err)
		return ReleasesData{}, err
	}
	var cacheData ReleasesData
	err = json.Unmarshal(content, &cacheData)
	if err != nil {
		fmt.Fprintln(os.Stderr, "Failed to unmarshal cache data:", err)
		return ReleasesData{}, err
	}
