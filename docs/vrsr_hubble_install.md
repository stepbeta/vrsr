...
Download and install hubble for the current OS/ARCH

### Synopsis

Download the hubble binary for the current OS/ARCH at the specified version.

This binary will be saved into the path specified by the "bin-path" flag. It will be named "hubble-$version".

Make sure to check the "use <version>" command after installing a new version

```
vrsr hubble install <version> [flags]
```
